...
class GomokuGame(QMainWindow):
    def __init__(self):
        ...
        # 初始化棋盤，0表示空，1表示玩家1（黑），-1表示玩家2（白）
        self.board = np.zeros((BOARD_SIZE, BOARD_SIZE), dtype=int)
        self.current_player = 1  # 玩家1（黑）先手
        ...
        # 設置標籤顯示當前玩家
        self.label = QLabel("玩家 1 (黑) 的回合", self)
        ...
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                ...
                button.setStyleSheet("background-color: #D2B48C; border: 1px solid black; border-radius: 20px;")
    ...
    def make_move(self, row, col):
        ...
        if self.current_player == 1:
            button.setStyleSheet("background-color: black; border: 1px solid black; border-radius: 20px;")
            self.label.setText("玩家 2 (白) 的回合")
        else:
            button.setStyleSheet("background-color: white; border: 1px solid black; border-radius: 20px;")
            self.label.setText("玩家 1 (黑) 的回合")

        # 檢查是否有玩家獲勝
        if self.check_win(self.current_player):
            self.label.setText(f"玩家 {1 if self.current_player == 1 else 2} 獲勝！")
